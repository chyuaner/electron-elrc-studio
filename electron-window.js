...
const fs = require("fs");
const { getWindowOptions, getShellConfig, TITLEBAR_COLORS } = require("./electron-shell-config");
...
function readShellAssets() {
  if (!cachedShellCss) {
    cachedShellCss = fs.readFileSync(path.join(__dirname, "electron-shell.css"), "utf8");
  }
  if (!cachedBootstrapJs) {
    cachedBootstrapJs = fs.readFileSync(path.join(__dirname, "electron-bootstrap.js"), "utf8");
  }
